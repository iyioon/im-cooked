...
import { RETRY_SETTINGS, RECIPE_SETTINGS } from "@/lib/constants";
  ...
  try {
    ...
    if (imageUrl && !imageUrl.startsWith("http")) {
      imageUrl = imageUrl.startsWith("//")
        ? `https:${imageUrl}`
        : `https://${new URL(url).hostname}${imageUrl}`;
    }
    ...
    const prepTimeEl = $(
      '[itemprop="prepTime"], .prep-time, .prepTime, .recipe-meta-item-header:contains("Prep")'
    ).first();
    if (prepTimeEl.length > 0) {
      prepTime =
        extractTime(prepTimeEl.text()) || extractTime(prepTimeEl.next().text()) || undefined;
    }

    // Extract cook time
    let cookTime: string | undefined = undefined;
    const cookTimeEl = $(
      '[itemprop="cookTime"], .cook-time, .cookTime, .recipe-meta-item-header:contains("Cook")'
    ).first();
    if (cookTimeEl.length > 0) {
      cookTime =
        extractTime(cookTimeEl.text()) || extractTime(cookTimeEl.next().text()) || undefined;
    }

    // Extract servings
    let servings: string | undefined = undefined;
    const servingsEl = $(
      '[itemprop="recipeYield"], .servings, .yield, .recipe-meta-item-header:contains("Servings")'
    ).first();
    if (servingsEl.length > 0) {
      servings =
        extractServings(servingsEl.text()) ||
        extractServings(servingsEl.next().text()) ||
        undefined;
    }
    ...
    return {
      ...
      title: title.substring(0, RECIPE_SETTINGS.MAX_TITLE_LENGTH),
 ...
 */
async function scrapeAllRecipesSearch(query: string, maxResults: number = 10): Promise<string[]> {
  try {
    ...
    const recipePromises = recipeUrls.map((url) =>
      Promise.race([
        ...
        new Promise<null>((resolve) =>
          setTimeout(() => resolve(null), RETRY_SETTINGS.RECIPE_FETCH_TIMEOUT)
        ),
      ])
    );
...
export async function getRecipeDetail(recipeId: string): Promise<RecipeDetail | null> {
  try {
    ...
    if (jsonLdData) {
      console.log(
        `JSON-LD extraction successful: ${jsonLdData.ingredients.length} ingredients, ${jsonLdData.instructions.length} instructions`
      );
    ...
    console.log(
      `Extracted ${steps.length} steps, ${steps.filter((s) => s.imageUrl).length} with images`
    );
    ...
    return {
      ...
      ingredients:
        ingredients.length > 0
          ? ingredients
          : [
              "Unable to extract ingredients from this page. This may be a collection page or the page structure is not supported.",
            ],
      instructions:
        instructions.length > 0
          ? instructions
          : [
              "Unable to extract instructions from this page. This may be a collection page or the page structure is not supported.",
            ],
