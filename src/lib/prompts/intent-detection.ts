...
export function buildIntentDetectionPrompt(userMessage: string): string {
  return `You are an AI cooking assistant classifier. Your task is to determine the user's intent and classify their message into one of three categories.
...
4. If the message is completely unrelated to food or cooking → REJECT
5. When in doubt between RECIPE_SEARCH and GENERAL_FOOD_QUESTION:
   - If it's about making a specific dish → RECIPE_SEARCH
   - If it's about a technique or concept → GENERAL_FOOD_QUESTION
6. For RECIPE_SEARCH intents, extract and clean the recipe query (remove filler words like "find me", "I want", etc.)
