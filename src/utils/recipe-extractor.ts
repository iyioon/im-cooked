...
import { RECIPE_SETTINGS } from "@/lib/constants";

/**
 * Nutrition information extracted from recipes
 */
  ...
  $('script[type="application/ld+json"]').each((_, el) => {
    ...
    try {
      ...
      if (Array.isArray(recipeData.recipeInstructions)) {
        recipeData.recipeInstructions.forEach((inst: JsonLdInstruction | string) => {
          ...
          if (inst["@type"] === "HowToSection") {
            ...
            if (Array.isArray(inst.itemListElement)) {
              inst.itemListElement.forEach((step: JsonLdStep) => {
          ...
          // Handle ItemList of HowToStep objects (CRITICAL - Bon Appétit, Epicurious, Serious Eats)
            ...
            inst.itemListElement.forEach((step: JsonLdStep | string) => {
        ...
        const cleaned = recipeData.recipeInstructions
          .trim()
          .replace(/^Step\s+\d+:\s*/i, "")
          .trim();
        if (cleaned) instructions.push(stripHtmlTags(cleaned));
 ...
 */
export function extractStepImages(
  $: cheerio.CheerioAPI,
  url: string,
  instructions: string[]
): RecipeStep[] {
  ...
  if (hostname.includes("allrecipes.com")) {
    ...
    $("h2, h3, h4").each((_, el) => {
      ...
      if (headingText === "directions" || headingText.startsWith("directions")) {
        ...
        while (nextEl.length > 0 && attempts < RECIPE_SETTINGS.MAX_HTML_TRAVERSAL_ATTEMPTS) {
        ...
        if (foundOl) {
          foundOl.find("li").each((idx, li) => {
            ...
            const text =
              p.length > 0
                ? p.text().trim()
                : $(li).clone().children("figure, img, div").remove().end().text().trim();
...
export function extractFromHtml($: cheerio.CheerioAPI): {
        ...
        while (nextEl.length > 0 && attempts < RECIPE_SETTINGS.MAX_HTML_TRAVERSAL_ATTEMPTS) {
          ...
          if (tagName === "ul" || tagName === "div") {
            nextEl.find("li").each((_, li) => {
              ...
              const hasControl =
                $(li).find('button, input[type="checkbox"], input[type="radio"]').length > 0;
              if (!hasControl) {
  ...
  $(
    '[itemprop="recipeInstructions"] li, [itemprop="recipeInstructions"] p, [itemprop="step"]'
  ).each((_, el) => {
    const text = $(el).text().trim();
    if (text && text.length > 0 && !instructions.includes(text)) {
  ...
  if (instructions.length === 0) {
    ...
    if (instructions.length === 0) {
      $(
        'ol li p, .directions li p, .instructions li p, .method li p, .steps li p, [class*="recipe-steps"] li p'
      ).each((_, el) => {
        const text = $(el).text().trim();
        if (text && text.length > 0 && !instructions.includes(text)) {
          instructions.push(stripHtmlTags(text));
